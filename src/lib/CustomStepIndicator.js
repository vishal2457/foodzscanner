...
export default class CustomStepIndicator extends PureComponent {
  constructor(props) {
    ...
    this.borderRadiusAnim = new Animated.Value(this.customStyles.stepIndicatorSize);
  }
  ...
  renderStepLabels = () => {
    ...
    var labelViews = labelItems.map((item,index) => {
      ...
      const selectedStepSubLabelStyle = index === currentPosition ? { color: this.customStyles.currentStepSubLabelColor } : { color: this.customStyles.subLabelColor }
      return (
        <TouchableOpacity
          activeOpacity={1}
        ...
        >
          <View style={[
            styles.stepLabelItem, {paddingBottom: 15},
            direction == 'vertical' ? {alignItems: 'flex-start'}: {alignItems: 'center'}
          ]}>
            <Text style={[
              styles.stepLabel, selectedStepLabelStyle,
            ...
            {item.subLabel &&
              <Text style={[
                styles.stepSubLabel, selectedStepSubLabelStyle,
              ]}>
                {item.subLabel}
              </Text>
            }
